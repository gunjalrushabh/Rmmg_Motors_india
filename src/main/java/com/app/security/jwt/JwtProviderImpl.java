...
public class JwtProviderImpl implements JwtProvider {
	...
	public Authentication getAuthentication(HttpServletRequest request) {
		...
		Set<GrantedAuthority> authoritiies = Arrays.stream(claims.get("roles").toString().split(","))
				.map(SecurityUtils::convertToAuthority).collect(Collectors.toSet());

		UserDetails userDetails = UserPrinciple.builder().username(username).authorities(authoritiies).id(userId)
				.build();

		if (username == null) {
			return null;
		}
		return new UsernamePasswordAuthenticationToken(userDetails, null, authoritiies);
	}
